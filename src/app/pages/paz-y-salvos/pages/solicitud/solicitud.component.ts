...
export class SolicitudComponent implements OnInit {
  ...
  obtenerInfoUsuario() {
    this.listService.getInfoEstudiante().then((resp) => {
      ...
      this.est.getEstudiantePorDocumento(resp.documento).subscribe((res) => {
        ...
        this.listService
          .loadFacultadProyectoTercero(this.estudiante.Id)
          .then((facultad) => {
            //cambiar el # por soliciante.terceroId
  ...
  guardarDatosFormulario() {
    ...
    this.referencia.MotivoAdministrativo =
      this.solicitarpyz.value.MotivoAdministrativo;
  ...
  guardarSolicitud() {
    ...
    solicitud.EstadoTipoSolicitudId = {
      Id: environment.IDS.IDTIPOSOLICITUDPAZYSALVOS,
    };
  ...
  grabarEvolucionSolicitud(solicitud: any) {
    ...
    evolucionSolicitud.FechaCreacion = this.dateCustomPipe.transform(
      new Date()
    );
    evolucionSolicitud.FechaModificacion = this.dateCustomPipe.transform(
      new Date()
    );
