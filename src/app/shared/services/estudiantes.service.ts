...
export class EstudiantesService {
  ...
  getEstudiante(codigo): Observable<any> {
  ...
  getInfoGrupoComplementaria(terceroId, Id) {
    return this.http.get(
      infoUrl + info + terceroId + grupoComplementaria + Id,
      headers
    );
  }
  getInfoComplementaria(terceroId, Id) {
    return this.http.get(
      infoUrl + info + terceroId + infoComplementaria + Id,
      headers
    );
  }
  getFacultades() {
  ...
  getEstudiantePorDocumento(document) {
    ...
    return this.rqManager
      .get(`datos_identificacion?query=Numero%3A${document}`)
      .pipe(
        map((res) => {
          if (res["Type"] === "error") {
            this.pUpManager.showErrorAlert(
              "No se Pudo registrar el rubro, compruebe que no exista un rubro con el mismo código o que este código sea admitido."
            );
            return undefined;
          }
          return res;
  ...
  grabarSolicitud(solicitud: any) {
    ...
    return this.rqManager.post("solicitud", solicitud).pipe(
      map((res) => {
        if (res["Type"] === "error") {
          this.pUpManager.showErrorAlert(
            "No se se pudo cargar la información."
          );
          return undefined;
        }
        return res;
